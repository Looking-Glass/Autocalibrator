...
boolean showDezPost=true;
FloatList[] dezHorizontalPost;
FloatList[] dezDiagonalPost;
FloatList[] derivativeHorizontalPost;
FloatList[] derivativeDiagonalPost;
...
void setup()
{
  ...
  run=int(lines[0])+1;
  loadData("run 46.csv");  //comment this line out to actually collect data from the sensors.  If we run loadData, it skips the data collection and goes straight to analysis, so you can run it without any hardware connected
...
void findPeaks()
{
  ...
  //for(int i=0;i<dezHorizontalPeaks[0].size();i++)
  // println(i+" "+dezHorizontalPeaks[0].get(i));

/*
  //postprocess
  for (int j=0; j<numPorts; j++)
 {
    float median = findMedian(dezHorizontalPeaks);
    dezHorizontalPost[j] = postProcess(dezHorizontalPeaks[j], numSlices);
  }
  // dezDiagonalPost
  // derivativeHorizontalPost
  // derivativeDiagonalPost;
